...
const LT=['kids','volwassenen','coordinator','redder','onthaalmedewerker','toezichter']
const LL={kids:'Kids',volwassenen:'Volwassenen',coordinator:'Coördinator',redder:'Redder',onthaalmedewerker:'Onthaalmedewerker',toezichter:'Toezichter'}
const MNL=['Januari','Februari','Maart','April','Mei','Juni','Juli','Augustus','September','Oktober','November','December']
const ROLES=['lesgever','coordinator','redder','onthaalmedewerker','toezichter']
const RC={lesgever:['#dbeafe','#1e3a8a'],coordinator:['#d1fae5','#064e3b'],redder:['#fee2e2','#991b1b'],onthaalmedewerker:['#fef3c7','#92400e'],toezichter:['#f3e8ff','#6b21a8']}
...
const Z=(id,n,k,v,c,r,ib='')=>({id,name:n,status:'zelfstandige',rates:{kids:k,volwassenen:v,coordinator:c||v,redder:r||0,onthaalmedewerker:12.5,toezichter:14},iban:ib})
const V=(id,n,k,v,c,r,o,ib='')=>({id,name:n,status:'vrijwilliger',rates:{kids:k,volwassenen:v,coordinator:c||0,redder:r||0,onthaalmedewerker:o||0,toezichter:14},iban:ib})
const INST=[
  Z('z1','Jasmin Husic',29,30,37,0),Z('z2','Jill Collier',20,20,0,20),Z('z3','Levi van Semang',34,34,35,0),
  Z('z4','Liesse Eyckmans',34,35,0,0),Z('z5','Lieze Dhondt',29,30,0,0),Z('z6','Michiel Willems',29,35,0,0),
  Z('z7','Michael Paridaens',34,35,0,0),Z('z8','Raf Plinke',34,35,35,0),Z('z9','Sami Baghouri',29,30,0,0),
  Z('z10','Ylana Longueville',32,35,0,25),Z('z11','Yosse Claessen',29,30,0,0),
  V('v1','Abdellah Atrari Marzouk',20,30),V('v2','Abraham Zeresenay',20,30,0,0,0,'BE25 3631 9843 7182'),
  V('v3','Amber Van Minnebruggen',20,30,0,0,0,'BE21 7506 9110 9503'),V('v4','Amber Willems',30,30),
  V('v5','Ann Pauwels',30,30),V('v6','Antonio Neto',30,30,30,10,0,'BE56 9501 1517 3788'),
  V('v7','Arthur Demey',23,30),V('v8','Blerim B.',30,30,30,10),
  V('v9','Bram Kruytzer',30,30),V('v10','Bram Wegge',30,30),V('v11','Broes Geens',26,30),
  V('v12','Bruno Engelen',30,30,30),V('v13','Celine Vranckx',30,30),
  V('v14','Charlotte Van den Camp',20,30),V('v15','Christophe Van Dyck',26,30),
  V('v16','Daan De Prins',23,30),V('v17','Damon van der Goten',30,30,30),
  V('v18','Daphne Peeters',30,30),V('v19','Deborah Bocken',23,30),
  V('v20','Dries Cornette',20,30),V('v21','Elia Schoonvliet',30,30,30),
  V('v22','Emilia Nelissen',26,30),V('v23','Flavio Claassen',26,30),
  V('v24',"Fleur D'heer",26,30),V('v25','Gerhele Hek',23,30,0,0,12.5),
  V('v26','Haroun Elkarmoudi',26,30,30),V('v27','Ilijana Jeremic',20,30),
  V('v28','Indy Visscher',30,30),V('v29','Isabel Buron',23,30),
  V('v30','Jelka Devriendt',30,30),V('v31','Jens Schoofs',23,30,0,10),
  V('v32','Jill Maene',30,30),V('v33','Julia Leenders',30,30),
  V('v34','Kaya de Backer',30,30,30),V('v35','Keela Demeyer',30,30),
  V('v36','Lana De Brueck',26,30,0,0,12.5),V('v37','Lili Boumans',20,20,0,20),
  V('v38','Lotte De Rechter',23,30),V('v39','Lukas Borghgraef',20,30),
  V('v40','Maaike Verbeke',30,30),V('v41','Marte Peeters',26,30),
  V('v42','Maya Arias',30,30),V('v43','Mustafa Al Tuwaijari',20,20,0,20),
  V('v44','Naomi Stabel',30,30,0,10),V('v45','Natascha VDS',30,30),
  V('v46','Nicolas Rottiers',20,30,0,0,12.5),V('v47','Niels Huybrechts',23,30,0,0,12.5),
  V('v48','Robbe Van Rie',20,30),V('v49','Soukaina Bounida',20,30,0,0,12.5),
  V('v50','Sterre Rymenants',20,30),V('v51','Swing Buytaert',20,30),
  V('v52','Tom Van den Bleeken',30,30,30,10),V('v53','Ubi De Roey',30,30),
  V('v54','Veerle Willems',20,30),V('v55','Wally Schoofs',30,30,0,10),
  V('v56','Wendy Dillen',26,30,30),V('v57','Younes Mazoud',23,30),V('v58','Kathleen Verbeke',23,30),
]
...
export default function App(){
  ...
  useEffect(()=>{
    loadAll()
    // Real-time: listen for changes from other users
    const channel=supabase.channel('swimpay-realtime')
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'app_data'},payload=>{
        const{id,value}=payload.new
        const parsed=JSON.parse(value)
        if(id==='sw_inst')setInst(parsed)
        else if(id==='sw_ent')setEntries(parsed)
        else if(id==='sw_sched')setSched(parsed)
        else if(id==='sw_att')setAtt(parsed)
        else if(id==='sw_comm')setComm(parsed)
  ...
  async function loadAll(){
    setSyncing(true)
    const[i,e,sc,a,c]=await Promise.all([dbGet('sw_inst'),dbGet('sw_ent'),dbGet('sw_sched'),dbGet('sw_att'),dbGet('sw_comm')])
    if(i)setInst(i);if(e)setEntries(e);if(sc)setSched(sc);if(a)setAtt(a);if(c)setComm(c)
    setSyncing(false);setLastSync(new Date())
  }

  const sv=async(k,d,fn)=>{fn(d);await dbSet(k,d)}

  const TABS=[{k:'dashboard',l:'Dashboard'},{k:'overzicht',l:'Overzicht lessen'},{k:'uren',l:'Uren invoeren'},{k:'maand',l:'Maandoverzicht'},{k:'lsg',l:'Lesgevers'}]
  return(
    <div style={{fontFamily:"system-ui,sans-serif",minHeight:'100vh',background:'#f8fafc'}}>
      <header style={{background:'#0f2133',display:'flex',alignItems:'center',height:52,paddingLeft:22,gap:2,position:'sticky',top:0,zIndex:50}}>
        ...
        {TABS.map(t=><button key={t.k} onClick={()=>setTab(t.k)} style={{padding:'0 13px',height:52,border:'none',cursor:'pointer',fontSize:13,fontWeight:500,fontFamily:'inherit',borderBottom:`2px solid ${tab===t.k?'#2dd4bf':'transparent'}`,background:'transparent',color:tab===t.k?'#2dd4bf':'rgba(255,255,255,0.55)'}}>{t.l}</button>)}
        <div style={{marginLeft:'auto',marginRight:14,display:'flex',alignItems:'center',gap:10}}>
          {syncing&&<span style={{fontSize:11,color:'#5eead4',animation:'pulse 1s infinite'}}>⟳ Syncing...</span>}
          ...
          <button onClick={loadAll} style={{background:'none',border:'1px solid rgba(255,255,255,0.2)',borderRadius:6,padding:'3px 8px',color:'rgba(255,255,255,0.5)',cursor:'pointer',fontSize:12}}>↻</button>
        </div>
      </header>
      <main style={{padding:'22px 26px',maxWidth:1200,margin:'0 auto'}}>
        ...
        {tab==='overzicht'&&<Overzicht sched={sched} inst={inst} entries={entries} att={att} onSaveEntries={d=>sv('sw_ent',d,setEntries)} onSaveAtt={d=>sv('sw_att',d,setAtt)} onSaveSched={d=>sv('sw_sched',d,setSched)}/>}
        {tab==='uren'&&<Uren inst={inst} entries={entries} onSave={d=>sv('sw_ent',d,setEntries)}/>}
        {tab==='maand'&&<Maand inst={inst} entries={entries}/>}
...
function Dashboard({sched,att,inst,comm,onSaveComm,onSaveAtt}){
  ...
  const allAbsences=useMemo(()=>{
    const result=[]
    Object.keys(att).forEach(wk=>{Object.keys(att[wk]).forEach(locId=>{const locData=sched.find(l=>l.locId===locId);if(!locData)return;Object.keys(att[wk][locId]).forEach(sessId=>{const sess=locData.sessions.find(s=>s.id===sessId);if(!sess)return;const sessDate=getDayDate(wk,sess.dag);Object.keys(att[wk][locId][sessId]).forEach(name=>{const a=att[wk][locId][sessId][name];if(a.aanwezig===false){const key=`${wk}_${locId}_${sessId}_${name}`;result.push({key,name,locId,locName:locData.name,sessId,dag:sess.dag,type:sess.type,date:sessDate,week:wk,sub:a.sub||''})}})})})})
    return result.sort((a,b)=>a.date.localeCompare(b.date))
  },[att,sched])
  const monthStr=`${viewYear}-${String(viewMonth+1).padStart(2,'0')}`
  const monthAbs=allAbsences.filter(a=>a.date.startsWith(monthStr))
  const commRows=allAbsences.filter(a=>!comm[a.key]?.done)
  const getC=key=>comm[key]||{overeenkomst:false,contactOuders:false,done:false}
  const setC=(key,patch)=>onSaveComm({...comm,[key]:{...getC(key),...patch}})
  const confirmC=key=>onSaveComm({...comm,[key]:{...getC(key),done:true}})
  const updateSub=(key,newSub)=>{
    ...
    next[wk][locId][sessId][name]={...(next[wk][locId][sessId][name]||{}),aanwezig:false,sub:newSub}
    onSaveAtt(next)
  }
  const consecutiveAbs=useMemo(()=>{
    const groups={};monthAbs.forEach(a=>{const k=`${a.name}||${a.locId}||${a.sessId}`;if(!groups[k])groups[k]={name:a.name,locName:a.locName,dag:a.dag,type:a.type,weeks:[],subs:[]};groups[k].weeks.push(a.week);groups[k].subs.push(a.sub)})
    return Object.values(groups).filter(g=>{if(g.weeks.length<2)return false;const s=[...g.weeks].sort();for(let i=1;i<s.length;i++){if(addDays(s[i-1],7)===s[i])return true}return false}).sort((a,b)=>b.weeks.length-a.weeks.length)
  },[monthAbs])
  const singleAbs=useMemo(()=>{
    const groups={};monthAbs.forEach(a=>{const k=`${a.name}||${a.locId}||${a.sessId}`;if(!groups[k])groups[k]={name:a.name,locName:a.locName,dag:a.dag,type:a.type,weeks:[],subs:[]};groups[k].weeks.push(a.week);groups[k].subs.push(a.sub)})
    return Object.values(groups).filter(g=>g.weeks.length===1).sort((a,b)=>a.name.localeCompare(b.name))
  },[monthAbs])
  const commOpen=commRows.filter(a=>{const c=getC(a.key);return!c.overeenkomst||!c.contactOuders}).length
  return(<div>
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:18}}>
      <MetCard label="Communicatie open" val={commOpen} color={commOpen>0?'#7c3aed':'#16a34a'} sub="overeenkomst / oudercontact"/>
      <MetCard label="Opeenvolgende uitval" val={consecutiveAbs.length} color={consecutiveAbs.length>0?'#d97706':'#16a34a'} sub={MNL[viewMonth]}/>
      <MetCard label="Enkelvoudige afwezigheid" val={singleAbs.length} color="#475569" sub={MNL[viewMonth]}/>
    </div>
    <div style={S.card}>
      ...
      <p style={{fontSize:12,color:'#64748b',marginBottom:14}}>Vervanger invullen hier synchroniseert direct naar <b>Overzicht lessen</b>. Beide groen → bevestigen.</p>
      {commRows.length===0
        ?<p style={{color:'#94a3b8',fontSize:13,textAlign:'center',padding:'14px 0'}}>✓ Alle vervangingen zijn volledig afgehandeld</p>
        :<div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
            <thead><tr style={{background:'#0f2133'}}>{['Datum','Dag','Locatie','Sessie','Afwezige','Vervanger','Overeenkomst','Contact ouders',''].map(h=><th key={h} style={{...S.th,color:'rgba(255,255,255,0.65)',padding:'10px 14px'}}>{h}</th>)}</tr></thead>
            <tbody>{commRows.map((a,i)=>{
              const c=getC(a.key);const canConfirm=c.overeenkomst&&c.contactOuders
              return(<tr key={a.key} style={{borderBottom:'1px solid #f1f5f9',background:i%2?'#fafcff':'#fff'}}>
                ...
                <td style={{...S.td,minWidth:180}}>
                  <div style={{display:'flex',alignItems:'center',gap:6}}>
                    <input list={`sub-dl-${i}`} value={a.sub} onChange={e=>updateSub(a.key,e.target.value)} placeholder="Naam vervanger..." style={{...S.inp,width:165,padding:'5px 9px',background:a.sub?'#f0fdf4':'#fff8ed',borderColor:a.sub?'#86efac':'#fb923c',color:a.sub?'#166534':'#92400e'}}/>
                    <datalist id={`sub-dl-${i}`}>{inames.map(n=><option key={n} value={n}/>)}</datalist>
                    {a.sub&&<span style={{fontSize:11,color:'#16a34a',fontWeight:600}}>✓</span>}
                  </div>
                </td>
                ...
                <td style={{...S.td,whiteSpace:'nowrap'}}>
                  <button onClick={()=>canConfirm&&confirmC(a.key)} title={canConfirm?'Bevestig':'Eerst beide afvinken'} style={{padding:'5px 12px',borderRadius:7,border:'none',cursor:canConfirm?'pointer':'not-allowed',fontWeight:600,fontSize:12,fontFamily:'inherit',background:canConfirm?'#0f2133':'#f1f5f9',color:canConfirm?'#2dd4bf':'#cbd5e1',whiteSpace:'nowrap'}}>{canConfirm?'✓ Bevestigen':'Wacht op beide'}</button>
                </td>
              </tr>)
            })}</tbody>
          </table>
    ...
    <div style={S.card}>
      ...
      <p style={{fontSize:12,color:'#64748b',marginBottom:12}}>Meerdere weken op rij afwezig → ouders informeren + vaste vervanger zoeken.</p>
      {consecutiveAbs.length===0?<p style={{color:'#94a3b8',fontSize:13,textAlign:'center',padding:'12px 0'}}>✓ Geen opeenvolgende uitval</p>:
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {consecutiveAbs.map((g,i)=>(<div key={i} style={{borderRadius:10,border:'2px solid #fcd34d',background:'#fffbeb',padding:'11px 14px',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
            <div style={{minWidth:150}}><div style={{fontWeight:700,fontSize:14}}>{g.name}</div><div style={{fontSize:12,color:'#64748b'}}>{g.locName} · {g.dag} · <TypeBadge type={g.type}/></div></div>
            <div style={{display:'flex',gap:5,flex:1,flexWrap:'wrap'}}>{[...g.weeks].sort().map((wk,j)=>(<span key={j} style={{padding:'4px 10px',borderRadius:20,fontSize:12,fontWeight:600,background:g.subs[j]?'#dcfce7':'#fee2e2',color:g.subs[j]?'#166534':'#991b1b',border:`1px solid ${g.subs[j]?'#86efac':'#fca5a5'}`}}>{fmtShort(getDayDate(wk,g.dag))} {g.subs[j]?'→ '+g.subs[j]:'⚠ open'}</span>))}</div>
            <span style={{padding:'4px 10px',borderRadius:20,fontSize:12,fontWeight:700,background:'#fef3c7',color:'#d97706',whiteSpace:'nowrap'}}>{g.weeks.length}× afwezig</span>
          </div>))}
        </div>
      }
    </div>
    {singleAbs.length>0&&(<div style={S.card}>
      ...
      <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
        ...
        <tbody>{singleAbs.map((g,i)=>(<tr key={i} style={{borderBottom:'1px solid #f8fafc',background:i%2?'#fafcff':'#fff'}}>
          ...
          <td style={S.td}>{g.subs[0]?<span style={{padding:'3px 8px',borderRadius:20,fontSize:12,fontWeight:600,background:'#dcfce7',color:'#166534'}}>{g.subs[0]}</span>:<span style={{padding:'3px 8px',borderRadius:20,fontSize:12,fontWeight:600,background:'#fee2e2',color:'#dc2626'}}>Geen vervanger</span>}</td>
        </tr>))}</tbody>
      </table></div>
    </div>)}
  </div>)
...
function Overzicht({sched,inst,entries,att,onSaveEntries,onSaveAtt,onSaveSched}){
  ...
  const getAtt=(sessId,name,duur)=>{const base=att[week]?.[loc]?.[sessId]?.[name];const defH=parseDuur(duur).toString();if(!base)return{aanwezig:true,sub:'',hours:defH};return{aanwezig:base.aanwezig!==false,sub:base.sub||'',hours:base.hours!==undefined&&base.hours!==''?base.hours:defH}}
  const setMA=(sessId,name,duur,patch)=>{const next=JSON.parse(JSON.stringify(att));if(!next[week])next[week]={};if(!next[week][loc])next[week][loc]={};if(!next[week][loc][sessId])next[week][loc][sessId]={};next[week][loc][sessId][name]={...getAtt(sessId,name,duur),...patch};onSaveAtt(next)}
  const isConf=sessId=>entries.some(e=>e._sessRef===`${week}_${loc}_${sessId}`)
  const confirmSess=(sess)=>{const date=getDayDate(week,sess.dag),locName=curLoc.name,newE=[];sess.members.forEach(m=>{const a=getAtt(sess.id,m.name,sess.duur);const lt=m.role==='lesgever'?sess.type:m.role;if(a.aanwezig!==false&&parseFloat(a.hours||0)>0){const f=inst.find(i=>i.name===m.name);if(f)newE.push({id:uid(),instId:f.id,date,loc:locName,lt,hours:parseFloat(a.hours),note:'',_sessRef:`${week}_${loc}_${sess.id}`})}else if(!a.aanwezig&&a.sub&&parseFloat(a.hours||0)>0){const f=inst.find(i=>i.name===a.sub);if(f)newE.push({id:uid(),instId:f.id,date,loc:locName,lt,hours:parseFloat(a.hours),note:`Vervanger voor ${m.name}`,_sessRef:`${week}_${loc}_${sess.id}`})}});if(newE.length){onSaveEntries([...entries,...newE]);alert(`✓ ${newE.length} uren bevestigd`)}else alert('Geen uren om te bevestigen.')}
  ...
  return(<div>
    ...
    {curLoc&&(<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(330px,1fr))',gap:13}}>
      {sorted.map(sess=>{
        const sessDate=getDayDate(week,sess.dag);const tc=TC[sess.type]||['#f3e8ff','#4c1d95']
        const confirmed=isConf(sess.id);const absCnt=sess.members.filter(m=>getAtt(sess.id,m.name,sess.duur).aanwezig===false).length
        return(<div key={sess.id} style={{background:'#fff',borderRadius:12,border:`1.5px solid ${confirmed?'#86efac':absCnt>0?'#fcd34d':'#e2e8f0'}`,overflow:'hidden'}}>
          <div style={{padding:'9px 13px',display:'flex',justifyContent:'space-between',alignItems:'center',background:confirmed?'#f0fdf4':absCnt>0?'#fffbeb':'#f8fafc',borderBottom:'1px solid #f1f5f9'}}>
            <div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}>
              <span style={{fontWeight:700,fontSize:14}}>{sess.dag}</span>
              <span style={{fontSize:12,color:'#64748b'}}>{fmtShort(sessDate)}</span>
              ...
              {absCnt>0&&!confirmed&&<span style={{fontSize:11,fontWeight:600,color:'#d97706',background:'#fef3c7',padding:'2px 6px',borderRadius:20}}>{absCnt} afwezig</span>}
              {confirmed&&<span style={{fontSize:11,color:'#16a34a',fontWeight:700}}>✓ bevestigd</span>}
            </div>
          ...
          <div style={{padding:'9px 13px'}}>
            {sess.members.map((m,i)=>{const a=getAtt(sess.id,m.name,sess.duur);const present=a.aanwezig!==false;return(
              <div key={i} style={{marginBottom:6,display:'flex',alignItems:'center',gap:5,flexWrap:'wrap'}}>
                ...
                <button onClick={()=>setMA(sess.id,m.name,sess.duur,{aanwezig:!present})} style={{padding:'4px 11px',borderRadius:20,border:'none',cursor:'pointer',fontWeight:600,fontSize:13,fontFamily:'inherit',background:present?'#dbeafe':'#fee2e2',color:present?'#1e3a8a':'#991b1b'}}>{m.name}</button>
                ...
                {!present&&<input list={`sl-${sess.id}-${i}`} placeholder="Vervanger..." value={a.sub} onChange={e=>setMA(sess.id,m.name,sess.duur,{sub:e.target.value})} style={{...S.inp,width:155,background:'#fff8ed',borderColor:a.sub?'#22c55e':'#fb923c',color:'#92400e',padding:'3px 10px'}}/>}
                <datalist id={`sl-${sess.id}-${i}`}>{sess.substitutes.map(s=><option key={s} value={s}/>)}{inames.map(n=><option key={n} value={n}/>)}</datalist>
              </div>
            )})}
            {!editMode&&!confirmed&&sess.members.length>0&&<button onClick={()=>confirmSess(sess)} style={{...S.btnP,marginTop:7,width:'100%',background:'#0d9488',fontSize:12,padding:'6px'}}>✓ Bevestigen → Maandoverzicht</button>}
            {confirmed&&<p style={{fontSize:12,color:'#16a34a',marginTop:5,textAlign:'center',fontWeight:600}}>✓ Uren opgenomen</p>}
          </div>
        </div>)
      })}
    </div>)}
    {editId&&sf&&(<div style={{position:'fixed',inset:0,background:'rgba(10,20,35,0.72)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:20}}>
      <div style={{background:'#fff',borderRadius:14,padding:24,width:'100%',maxWidth:510,maxHeight:'88vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.25)'}}>
        ...
        <label style={{...S.lbl,marginBottom:7}}>Vaste lesgevers</label>
...
function Uren({inst,entries,onSave}){
  ...
  return(<div>
    <Title t="Uren invoeren" s="Manuele invoer — bevestigde uren via Overzicht lessen worden hier ook opgenomen."/>
    <div style={S.card}>
      ...
      {rows.length===0?<p style={{textAlign:'center',color:'#94a3b8',padding:'20px 0',fontSize:13}}>Geen uren.</p>:(
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12.5}}>
          ...
          <tbody>{rows.map((e,i)=>{const ins=gi(e.instId);return(<tr key={e.id} style={{borderBottom:'1px solid #f8fafc',background:i%2?'#fafcff':'#fff'}}>
            <td style={S.td}>{e.date}</td><td style={{...S.td,fontWeight:500}}>{ins?ins.name:'?'}</td><td style={S.td}>{e.loc}</td><td style={S.td}>{LL[e.lt]}</td>
...
function Maand({inst,entries}){
  ...
  const exportXL=()=>{const mn=MNL[mo],d=[];d.push([`Uitbetalingen ${mn} ${yr}`],[]);const hdr=['Naam','IBAN',...LT.flatMap(t=>[LL[t]+' u',LL[t]+' €']),'TOTAAL €'];['zelfstandige','vrijwilliger'].forEach(st=>{d.push([st==='zelfstandige'?'ZELFSTANDIGEN':'VRIJWILLIGERS']);d.push(hdr);const rs=rows(st);if(!rs.length){d.push(['(geen)']);d.push([]);return}const th={},ta={};LT.forEach(t=>{th[t]=0;ta[t]=0});rs.forEach(({i,s,tot})=>{const row=[i.name,i.iban||''];LT.forEach(t=>{row.push(s[t].h||0);row.push(+(s[t].a||0).toFixed(2));th[t]+=s[t].h||0;ta[t]+=s[t].a||0});row.push(+tot.toFixed(2));d.push(row)});const tr=['TOTAAL',''];LT.forEach(t=>{tr.push(th[t]);tr.push(+ta[t].toFixed(2))});tr.push(+LT.reduce((s,t)=>s+ta[t],0).toFixed(2));d.push(tr);d.push([])});const ws=XLSX.utils.aoa_to_sheet(d);ws['!cols']=[{wch:26},{wch:22},...LT.flatMap(()=>[{wch:8},{wch:11}]),{wch:12}];const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,`${mn} ${yr}`);XLSX.writeFile(wb,`Uitbetalingen_${mn}_${yr}.xlsx`)}
  const Sec=({st})=>{const rs=rows(st);const gt=rs.reduce((s,r)=>s+r.tot,0);if(!rs.length)return null;return(<div style={{marginBottom:24}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}><SBadge s={st}/><span style={{fontSize:13,color:'#64748b'}}>{rs.length} personen</span><span style={{fontWeight:700,color:'#0d9488'}}>{euro(gt)}</span></div>
    <div style={{overflowX:'auto',borderRadius:10,border:'1px solid #e2e8f0'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
      <thead><tr style={{background:'#0f2133'}}><th style={{...S.th,color:'rgba(255,255,255,0.65)',minWidth:130}}>Naam</th>{LT.map(t=><th key={t} colSpan={2} style={{...S.th,color:'rgba(255,255,255,0.65)',textAlign:'center',borderLeft:'1px solid rgba(255,255,255,0.1)'}}>{LL[t]}</th>)}<th style={{...S.th,color:'#2dd4bf',textAlign:'right'}}>Totaal</th></tr>
        <tr style={{background:'#162840'}}><td style={{padding:'3px 14px'}}/>{LT.map(t=>[<td key={t+'u'} style={{padding:'3px 8px',fontSize:10,color:'rgba(255,255,255,0.3)',borderLeft:'1px solid rgba(255,255,255,0.07)'}}>uren</td>,<td key={t+'e'} style={{padding:'3px 8px',fontSize:10,color:'rgba(255,255,255,0.3)'}}>€</td>])}<td/></tr>
      </thead>
      <tbody>{rs.map(({i,s,tot},idx)=>(<tr key={i.id} style={{background:idx%2?'#fafcff':'#fff',borderBottom:'1px solid #f1f5f9'}}>
        ...
        {LT.map(t=>[<td key={t+'h'} style={{...S.td,color:s[t].h?'#334155':'#e2e8f0',borderLeft:'1px solid #f1f5f9'}}>{s[t].h?s[t].h+'u':'—'}</td>,<td key={t+'a'} style={{...S.td,textAlign:'right',color:s[t].a?'#0d9488':'#e2e8f0',fontWeight:s[t].a?600:400}}>{s[t].a?euro(s[t].a):'—'}</td>])}
  ...
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
      <Title t="Maandoverzicht" s="Overzicht per maand — klaar voor export."/>
    ...
    {me.length===0?<div style={{...S.card,textAlign:'center',padding:'44px 0',color:'#94a3b8'}}><div style={{fontSize:30,marginBottom:8}}>📋</div><p style={{color:'#64748b',fontWeight:600,margin:'0 0 4px'}}>Geen uren voor {MNL[mo]} {yr}</p><p style={{fontSize:13}}>Bevestig uren via Overzicht lessen.</p></div>
...
function Lesgevers({inst,onSave}){
  ...
  const empty={name:'',status:'vrijwilliger',iban:'',rates:{kids:30,volwassenen:30,coordinator:0,redder:20,onthaalmedewerker:0,toezichter:14}}
  ...
  const list=inst.filter(i=>fst==='all'||i.status===fst).filter(i=>i.name.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>a.status!==b.status?(a.status==='zelfstandige'?-1:1):a.name.localeCompare(b.name))
  const ii={...S.inp,width:'100%',padding:'8px 10px',boxSizing:'border-box'}
  return(<div>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}><Title t="Lesgevers" s={`${inst.length} lesgevers — tarieven en gegevens`}/><button onClick={openAdd} style={S.btnP}>+ Nieuwe lesgever</button></div>
    <div style={{display:'flex',gap:10,marginBottom:12}}><input placeholder="Zoeken..." value={search} onChange={e=>setSearch(e.target.value)} style={{...ii,width:200}}/><select value={fst} onChange={e=>setFst(e.target.value)} style={{...ii,width:'auto',padding:'8px 12px'}}><option value="all">Alle</option><option value="zelfstandige">Zelfstandigen</option><option value="vrijwilliger">Vrijwilligers</option></select></div>
    <div style={{...S.card,padding:0,overflow:'hidden'}}><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12.5}}>
      <thead><tr style={{background:'#f8fafc',borderBottom:'1px solid #e2e8f0'}}>{['Naam','Statuut','Kids','Volwas','Coord','Redder','Onthaal','Toezicht','IBAN',''].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead>
      <tbody>{list.map((i,idx)=>(<tr key={i.id} style={{borderBottom:'1px solid #f8fafc',background:idx%2?'#fafcff':'#fff'}}>
        ...
        {['kids','volwassenen','coordinator','redder','onthaalmedewerker','toezichter'].map(t=><td key={t} style={{...S.td,color:i.rates[t]?'#475569':'#e2e8f0'}}>{i.rates[t]?'€'+i.rates[t]:'—'}</td>)}
        <td style={{...S.td,fontFamily:'monospace',fontSize:11,color:'#94a3b8'}}>{i.iban||'—'}</td>
        <td style={{...S.td,whiteSpace:'nowrap'}}><button onClick={()=>openEdit(i)} style={{...S.btnSm,marginRight:5}}>✏</button>{del===i.id?<span style={{display:'inline-flex',gap:4}}><button onClick={()=>{onSave(inst.filter(x=>x.id!==i.id));setDel(null)}} style={{...S.btnSm,background:'#fee2e2',color:'#dc2626'}}>Ja</button><button onClick={()=>setDel(null)} style={S.btnSm}>Nee</button></span>:<button onClick={()=>setDel(i.id)} style={{background:'none',border:'none',cursor:'pointer',color:'#cbd5e1',fontSize:17,padding:'0 4px'}}>×</button>}</td>
      </tr>))}</tbody>
    ...
    {modal&&f&&(<div style={{position:'fixed',inset:0,background:'rgba(10,20,35,0.7)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:20}}>
      <div style={{background:'#fff',borderRadius:14,padding:24,width:'100%',maxWidth:475,maxHeight:'88vh',overflowY:'auto',boxShadow:'0 20px 60px rgba(0,0,0,0.25)'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:12}}>{LT.map(t=><div key={t}><label style={{...S.lbl,fontSize:10}}>{LL[t]}</label><input type="number" step="0.5" min="0" value={f.rates[t]||0} onChange={e=>setF(p=>({...p,rates:{...p.rates,[t]:parseFloat(e.target.value)||0}}))} style={ii}/></div>)}</div>
